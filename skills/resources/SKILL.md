...
| `medal.bookings.events` | `src/resources/bookings.ts` (`BookingsEvents`) | `list({ from, to, status?, host_id? })`, `get(id)`, `create(input)`, `register(id, input, opts?)`, `registrations(id)` — arrangementer (scheduled group sessions); `register` enrolls a child, optionally starting a Vipps payment; `registrations` reads the roster |
